...
def test_login_to_checkin(page:Page):
    ...
    with allure.step('check if customer in the checkin page'):
        expect(page).to_have_url(re.compile(".*check-in.*"))

    welcome_message = page.get_by_text('upcoming meeting')
    assert welcome_message.is_visible()
    screenshot_path = 'tests/check_in_page/screenshots/checkin_page.png.png'
